...
Email delivery is optional for password sign-in. Configure it for workspace invitations and the temporary legacy-account verification flow. Invitation links and verification codes are never written to the server log.
...
| `AUTH_LEGACY_AUTH_ENABLED` | `true` | Keeps legacy installed-client email-code and Google routes available only for existing passwordless accounts; set `false` after migration |
...
| `GOOGLE_CLIENT_ID` | empty | Google OAuth client ID used only by legacy installed clients while compatibility is enabled |
| `GOOGLE_CLIENT_SECRET` | empty | Matching Google OAuth client secret; keep in a deployment secret |
| `GOOGLE_REDIRECT_URI` | empty | Exact redirect URI registered for the legacy Google OAuth client |
...
| Variable | Default | Description |
|---|---|---|
| `AUTH_PASSWORD_EMAIL_LIMIT` | `5` | Durable attempts per email within the authentication window; stored as a keyed hash in PostgreSQL |
| `AUTH_PASSWORD_IP_LIMIT` | `20` | Durable attempts per client IP within the authentication window; stored as a keyed hash in PostgreSQL |
| `AUTH_PASSWORD_ATTEMPT_WINDOW` | `15m` | Window for durable password and legacy-auth attempt limits |
| `REDIS_URL` | empty | Optional auth burst limiter, realtime events, and token cache; durable authentication limits remain on when unset |
| `REDIS_DISABLE_CLIENT_NAME` | `false` | Set `true` when a managed Redis blocks `CLIENT SETNAME` |
| `RATE_LIMIT_AUTH` | `5` | Per-IP login and registration requests per minute |
...
The `RATE_LIMIT_*` variables apply only to the optional Redis burst limiter. PostgreSQL limits are enforced independently for password and legacy authentication, and fail closed if their database query cannot run. If Redis becomes temporarily unavailable, the burst limiter fails open while the durable limits remain active.
