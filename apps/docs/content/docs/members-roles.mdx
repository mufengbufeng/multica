...
The invitee opens the emailed invitation link, signs in or registers, and then accepts the invitation. The link is the proof of invitation; signing in with a matching email address alone does not grant access. Invitations are valid for 7 days; resend after they expire. On self-hosted instances that restrict signup, invitees who do not yet have an account are still subject to the signup rules when registering — see [Auth setup](/auth-setup#invitations-and-signup-restrictions).
