...
비밀번호 로그인에는 이메일 서비스가 필요하지 않습니다. 워크스페이스 초대와 임시 이전 계정 마이그레이션에는 이메일 전송이 필요합니다. 초대 링크와 인증 코드는 capability credential이므로 서버 로그에 기록되지 않습니다.

<Callout type="warning">
프로덕션 환경에서 기본 `JWT_SECRET`을 사용하지 마세요.
</Callout>
...
| `AUTH_LEGACY_AUTH_ENABLED` | `true` | 비밀번호가 없는 기존 계정에 대해 이전 설치형 클라이언트 이메일 코드 및 Google 경로를 유지; 마이그레이션 후 `false`로 설정 |
...
| `GOOGLE_CLIENT_ID` | 비어 있음 | 호환 기간 중 이전 설치형 클라이언트가 사용하는 Google OAuth client ID |
| `GOOGLE_CLIENT_SECRET` | 비어 있음 | 대응하는 Google OAuth client secret; 배포 secret에 보관 |
| `GOOGLE_REDIRECT_URI` | 비어 있음 | 이전 Google OAuth 클라이언트에 등록한 정확한 redirect URI |
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `AUTH_PASSWORD_EMAIL_LIMIT` | `5` | 인증 창 안에서 이메일별 지속 시도 수; PostgreSQL에 해시로 저장 |
| `AUTH_PASSWORD_IP_LIMIT` | `20` | 인증 창 안에서 클라이언트 IP별 지속 시도 수; PostgreSQL에 해시로 저장 |
| `AUTH_PASSWORD_ATTEMPT_WINDOW` | `15m` | 비밀번호 및 이전 인증에 대한 지속 rate limit 창 |
| `REDIS_URL` | 비어 있음 | 선택적 인증 burst limiter, 실시간 이벤트, token cache; 미설정이어도 지속 인증 limit은 동작 |
| `REDIS_DISABLE_CLIENT_NAME` | `false` | 관리형 Redis가 `CLIENT SETNAME`을 금지하면 `true`로 설정 |
| `RATE_LIMIT_AUTH` | `5` | IP당 1분에 로그인 또는 가입을 허용하는 횟수 |
...
`RATE_LIMIT_*` 변수는 선택적 Redis burst limiter만 제어합니다. 비밀번호 및 이전 인증은 PostgreSQL 지속 rate limit을 독립적으로 사용하며, 해당 데이터베이스 쿼리를 실행할 수 없으면 인증 요청을 거부합니다. Redis가 일시적으로 사용할 수 없으면 burst limiter는 요청을 허용하지만 지속 limit은 계속 동작합니다.
