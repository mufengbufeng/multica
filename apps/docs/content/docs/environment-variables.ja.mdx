...
パスワードによるログインにメール配信は不要です。ワークスペース招待と一時的な旧アカウント移行にはメール配信が必要です。招待リンクと認証コードは能力資格情報のため、サーバーログには書き込まれません。

<Callout type="warning">
プロダクションでデフォルトの `JWT_SECRET` を使わないでください。
</Callout>
...
| `AUTH_LEGACY_AUTH_ENABLED` | `true` | パスワードのない既存アカウント向けに、旧インストール済みクライアントのメールコードと Google ルートを維持。移行後は `false` に設定 |
...
| `GOOGLE_CLIENT_ID` | 空 | 互換期間中に旧インストール済みクライアントが使う Google OAuth client ID |
| `GOOGLE_CLIENT_SECRET` | 空 | 対応する Google OAuth client secret。デプロイ secret に保管 |
| `GOOGLE_REDIRECT_URI` | 空 | 旧 Google OAuth クライアントに登録した正確な redirect URI |
...
| 変数 | デフォルト | 説明 |
|---|---|---|
| `AUTH_PASSWORD_EMAIL_LIMIT` | `5` | 認証ウィンドウ内のメールアドレスごとの永続的な試行回数。PostgreSQL にハッシュで保存 |
| `AUTH_PASSWORD_IP_LIMIT` | `20` | 認証ウィンドウ内のクライアント IP ごとの永続的な試行回数。PostgreSQL にハッシュで保存 |
| `AUTH_PASSWORD_ATTEMPT_WINDOW` | `15m` | パスワードおよび旧認証の永続的なレート制限ウィンドウ |
| `REDIS_URL` | 空 | 任意の認証バースト制限、リアルタイムイベント、token cache。未設定でも永続的な認証制限は有効 |
| `REDIS_DISABLE_CLIENT_NAME` | `false` | マネージド Redis が `CLIENT SETNAME` を禁止している場合に `true` を設定 |
| `RATE_LIMIT_AUTH` | `5` | ログインまたは登録の、IP あたり毎分の回数 |
...
`RATE_LIMIT_*` は任意の Redis バースト制限だけを制御します。パスワードと旧認証は PostgreSQL の永続的なレート制限を独立して使い、そのクエリを実行できない場合は認証要求を拒否します。Redis が一時的に利用できない場合、バースト制限は通しますが、永続的な制限は有効なままです。
