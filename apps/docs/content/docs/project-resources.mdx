...
Desktop lets you pick a folder directly. In the browser, open the project's **Resources**, choose "Add local directory", select a local runtime you can manage, and enter the absolute path on that runtime's computer. Browsers cannot inspect that computer's filesystem; the target daemon validates the path before it uses it for a task.
...
2. Choose "Add local directory".
3. In Desktop, pick the folder to use. In the browser, select the target local runtime and enter that computer's absolute path.

The directory must be an absolute path, must already exist, and must be readable and writable by the target daemon. Desktop checks this when you choose the folder; browser bindings are checked by the target daemon before a task uses them. These paths are rejected: system roots and drive roots (`/`, `C:\`), home directories themselves and the parents of home directories (such as `/Users`, `/home`, `/root`), and system directories such as `/etc`, `/var`, `/tmp`, `/usr`, and `/opt`. If the chosen path is a symlink, it is resolved to its real path first and validated again by the same rules; the serial lock also applies to the real path.
...
To change an existing binding, use **Change directory** on its resource row. It updates that resource in place; in the browser you may also move it to another local runtime you can manage, as long as that daemon does not already have a directory for the project.
