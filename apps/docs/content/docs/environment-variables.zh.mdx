...
密码登录不需要邮件服务。工作区邀请和临时旧账号迁移需要邮件投递；邀请链接和验证码都是能力凭据，绝不会写入服务端日志。
...
| `AUTH_LEGACY_AUTH_ENABLED` | `true` | 为没有密码的已有账号保留旧安装版客户端的邮箱验证码和 Google 路由；迁移完成后设为 `false` |
| `LOG_LEVEL` | 应用默认 | 日志级别 |
...
| `GOOGLE_CLIENT_ID` | 空 | 兼容窗口内旧安装版客户端使用的 Google OAuth client ID |
| `GOOGLE_CLIENT_SECRET` | 空 | 对应的 Google OAuth client secret；请放入部署密钥 |
| `GOOGLE_REDIRECT_URI` | 空 | 旧 Google OAuth 客户端在 Google 注册的精确回调地址 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `AUTH_PASSWORD_EMAIL_LIMIT` | `5` | 单个邮箱在认证窗口内的持久化尝试次数；邮箱哈希存于 PostgreSQL |
| `AUTH_PASSWORD_IP_LIMIT` | `20` | 单个客户端 IP 在认证窗口内的持久化尝试次数；IP 哈希存于 PostgreSQL |
| `AUTH_PASSWORD_ATTEMPT_WINDOW` | `15m` | 密码和旧认证的持久化限流窗口 |
| `REDIS_URL` | 空 | 可选的认证突发限流、实时事件和 token cache；未设置时持久化认证限流仍然生效 |
| `REDIS_DISABLE_CLIENT_NAME` | `false` | 托管 Redis 禁止 `CLIENT SETNAME` 时设为 `true` |
| `RATE_LIMIT_AUTH` | `5` | 单 IP 每分钟登录或注册的次数 |
...
`RATE_LIMIT_*` 只控制可选的 Redis 突发限流。密码和旧认证独立使用 PostgreSQL 持久化限流；其数据库查询不可用时会拒绝认证请求。Redis 临时不可用时，突发限流会放行，但持久化限流仍然有效。
