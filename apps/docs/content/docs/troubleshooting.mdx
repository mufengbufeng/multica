...
Check the backend startup log first. It states whether the server is using `SMTP relay`, `Resend API`, or has email delivery disabled:
...
- **Email delivery disabled**: no email is sent, and the backend rejects new invitations rather than writing invitation links to logs.
...
When `SMTP_HOST` and Resend are both configured, Multica prefers SMTP. For configuration, see [Sign-in and signup](/auth-setup#email-delivery-for-invitations).
