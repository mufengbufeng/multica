...
打开本机的 `http://localhost:3000`，或刚才配置的 `https://app.example.com`，使用邮箱和密码创建账号，然后创建第一个工作区。

注册和登录不需要邮件服务。需要通过邮件发送工作区邀请时配置 Resend 或 SMTP；未配置时会拒绝新邀请，邀请链接绝不会写入 backend 日志。详见[登录与注册配置](/auth-setup)。
...
- [登录与注册配置](/auth-setup) — 配置邀请邮件和注册范围。
