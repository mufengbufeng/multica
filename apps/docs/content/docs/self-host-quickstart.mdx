...
Open `http://localhost:3000` locally, or the `https://app.example.com` you just configured. Create an account with your email address and a password, then create your first workspace.

No email service is needed to register or sign in. Configure Resend or SMTP when Multica should deliver workspace invitations by email; without it, new invitations are rejected and no invitation links are written to backend logs. See [Auth setup](/auth-setup).
...
| Cannot sign in | Verify the email address and password, then check the signup allowlist if registration is restricted. |
...
- [Auth setup](/auth-setup) — configure invitation email delivery and signup scope.
