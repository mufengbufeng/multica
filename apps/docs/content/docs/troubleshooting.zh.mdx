...
先查看 backend 启动日志。日志会说明当前使用 `SMTP relay`、`Resend API`，还是已禁用邮件投递：
...
- **已禁用邮件投递**：邮件不会发出，backend 会拒绝新邀请，而不会把邀请链接写入日志；
- **Resend**：确认 API key 有效，且发件地址的域名已经验证；
- **SMTP**：确认主机、端口、凭据和发件地址，并从错误日志判断失败发生在连接、TLS、认证还是投递阶段。

`SMTP_HOST` 和 Resend 同时配置时，Multica 会优先使用 SMTP。配置方式见[登录与注册](/auth-setup#用于邀请的邮件发送)。
