...
// Its default secret guard drops names containing KEY, SECRET, or TOKEN. The
// daemon therefore owns the shell policy in each task's isolated Codex home so
// the opaque MULTICA_TASK_CLI_CAPABILITY reaches normal `multica ...` shell
// commands without exposing the raw task token to the provider process.
const (
